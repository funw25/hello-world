...
bool conditional(bool p, bool q) { return !p || q; }
bool biconditional(bool p, bool q) { return p == q; }
bool xor(bool p, bool q) { return p != q; }
bool nand(bool p, bool q) { return !(p && q); }
bool nor(bool p, bool q) { return !(p || q); }
bool cond_neg(bool p, bool q) { return p && !q; }

// 打印二元逻辑运算表
void print_binary_table(FILE *fp, const char *name, bool (*op)(bool, bool)) {
    fprintf(fp, "------------------\n");
    fprintf(fp, " P   Q    %s\n", name);
    fprintf(fp, "------------------\n");
    for (int p = 1; p >= 0; p--) {
        for (int q = 1; q >= 0; q--) {
            char p_char = p ? 'T' : 'F';
            char q_char = q ? 'T' : 'F';
            char res_char = op(p, q) ? 'T' : 'F';
            fprintf(fp, " %c   %c    %c\n", p_char, q_char, res_char);
        }
    }
    fprintf(fp, "------------------\n\n");
}
...
// 打印一元逻辑运算表（非）
void print_unary_table(FILE *fp, const char *name, bool (*op)(bool)) {
    fprintf(fp, "------------------\n");
    fprintf(fp, " P    %s\n", name);
    fprintf(fp, "------------------\n");
    for (int p = 1; p >= 0; p--) {
        char p_char = p ? 'T' : 'F';
        char res_char = op(p) ? 'T' : 'F';
        fprintf(fp, " %c     %c\n", p_char, res_char);
    }
    fprintf(fp, "------------------\n\n");
}
...
int main() {
    FILE *fp = fopen("logic.txt", "w");
    if (fp == NULL) {
        printf("无法创建文件 logic.txt\n");
        return 1;
    }

    print_binary_table(fp, "AND", and);
    print_binary_table(fp, "OR", or);
    print_unary_table(fp, "NOT", not);
    print_binary_table(fp, "CONDITIONAL", conditional);
    print_binary_table(fp, "BICONDITIONAL", biconditional);
    print_binary_table(fp, "XOR", xor);
    print_binary_table(fp, "NAND", nand);
    print_binary_table(fp, "NOR", nor);
    print_binary_table(fp, "CONDITIONAL NEGATION", cond_neg);
